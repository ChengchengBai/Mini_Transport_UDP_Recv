...
int is_accept_ip_packet(struct ip_header *ip_hdr)
{
	...
	length_ip_hdr = length_ip_hdr >> 4;
	u_int16_t check_sum = calculate_check_sum(ip_hdr, length_ip_hdr * 4);//别人发多数为20 自己人发60
...
int network_ipv4_recv(u_int8_t *ip_buffer)
{
	...
	case IPPROTO_UDP:
		...
		binds(Servaddr);
		recvfroms(Servaddr,(data_buffer+length_ip_hdr),ip_hdr->total_length-length_ip_hdr,1,ip_hdr->source_ip,4);
		break;
	}
